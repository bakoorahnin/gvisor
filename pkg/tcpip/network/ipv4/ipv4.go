...
func (e *endpoint) forwardPacket(pkt *stack.PacketBuffer) ip.ForwardingError {
	...
	} else if header.IsV4LinkLocalUnicastAddress(dstAddr) || header.IsV4LinkLocalMulticastAddress(dstAddr) {
...
func (e *endpoint) handleValidatedPacket(h header.IPv4, pkt *stack.PacketBuffer) {
	...
	if addressEndpoint := e.AcquireAssignedAddress(dstAddr, e.nic.Promiscuous(), stack.CanBePrimaryEndpoint); addressEndpoint != nil {
		...
		if err := e.forwardPacket(pkt); err != nil {
			stats.ip.PacketForwardingErrors.Increment()
			switch err.(type) {
			case *ip.ErrLinkLocalSourceAddress:
				stats.ip.PacketsDroppedDueToLinkLocalSourceAddress.Increment()
			case *ip.ErrLinkLocalDestAddress:
				stats.ip.PacketsDroppedDueToLinkLocalDestAddress.Increment()
			case *ip.ErrTTLExceeded:
				stats.ip.PacketsDroppedDueToExhaustedTTL.Increment()
			case *ip.ErrCantCreateRoute:
				stats.ip.UnrouteablePacketsDropped.Increment()
