...
func newICMPv4EchoRequest(payloadSize int) buffer.View {
	buf := buffer.NewView(header.ICMPv4MinimumSize + payloadSize)
...
func TestWriteUnboundWithBindToDevice(t *testing.T) {
	// payloadSize is the size, in bytes, of the ICMP echo request payloads.
	const payloadSize = header.IPv4MinimumMTU - header.IPv4MinimumSize - header.ICMPv4MinimumSize
	...
	{
		buf := newICMPv4EchoRequest(payloadSize)
		r := buf.Reader()
		...
		// Verify the packet was sent out the higher priority NIC.
		p, ok := defaultEP.Read()
		if !ok {
			t.Fatalf("got highPriorityEP.Read(_) = _, false; want = _, true (packet wasn't written out)")
		}
		...
		// Verify the packet was not sent out the lower priority NIC.
		if p, ok := alternateEP.Read(); ok {
			t.Fatalf("got lowPriorityEP.Read(_) = %+v, true; want = _, false", p)
		}
	}
	...
	// SO_BINDTODEVICE to route the packet to the alternate NIC.
	{
		...
		// Verify the packet was not sent out the higher priority NIC.
		if p, ok := defaultEP.Read(); ok {
			t.Fatalf("got highPriorityEP.Read(_) = %+v, true; want = _, false", p)
		}

		// Verify the packet was sent out the lower priority NIC.
		p, ok := alternateEP.Read()
		if !ok {
			t.Fatalf("got lowPriorityEP.Read(_) = _, false; want = _, true (packet wasn't written out)")
		}
