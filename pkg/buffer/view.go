...
// Remove deletes data at specified location in v.
func (v *View) Remove(offset, length int) {
	...
	for buf := v.data.Front(); buf != nil; {
		...
		if x := curr.Intersect(tgt); x.Len() > 0 {
			buf.Remove(x.Offset(-curr.begin))
			if buf.ReadSize() == 0 {
				oldBuf := buf
				buf = buf.Next()
...
func (v *View) PullUp(offset, length int) ([]byte, bool) {
	...
	for n := buf; n != nil && off < totLen; {
		...
		// Remove buffers except for the first one.
		if n == buf {
			n = n.Next()
		} else {
			old := n
...
// SubApply applies fn to a given range of data in v.
...
// Offset returns x offsetted by off.
